...
// Generate a consistent accent color for any model name
...
async function loadPrediction(stock) {
    ...
    try {
        ...
        if (data.is_live === false) {
            // No live data — show "__" for all values and clear charts
            renderNoLiveData(data);
            clearAllCharts();
        } else {
            renderSummary(data);
...
// ===== NO LIVE DATA — show "__" =====
function renderNoLiveData(data) {
    ...
    document.getElementById('summary').innerHTML = `
        <div class="cards">
            ...
            <div class="card card--date">
                ...
                <span style="margin-left:auto;font-size:0.78rem;color:var(--text-muted);">${data.dataset || placeholder}</span>
            </div>

            <div class="card">
                <div class="card-label">Stock</div>
                <div class="card-value">${data.stock || placeholder}</div>
            </div>
            <div class="card">
                <div class="card-label">Open</div>
                <div class="card-value">${placeholder}</div>
            </div>
            <div class="card">
...
function clearAllCharts() {
    ...
    ['actualChart', 'residualChart', 'rollingAccChart', 'densityChart', 'directionChart'].forEach(id => {
